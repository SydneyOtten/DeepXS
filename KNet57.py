...
with open("K57.txt") as f2:
    LO=[x2.strip('\n') for x2 in f2.readlines()]

X2=np.empty([99999,15])
y1=np.ones(99999)

for j in range(0, 99999):
    X2[j,0]=LO[21*j] #mix1
    X2[j,1]=LO[21*j+1] #mix2
    X2[j,2]=LO[21*j+2] #mix3
    X2[j,3]=LO[21*j+3] #mix4
    X2[j,4]=LO[21*j+5] #mchar
    X2[j,5]=LO[21*j+6] #sq1
    X2[j,6]=LO[21*j+7] #sq2
    X2[j,7]=LO[21*j+8] #sq3
    X2[j,8]=LO[21*j+9] #sq4
    X2[j,9]=LO[21*j+10] #sq5
    X2[j,10]=LO[21*j+11] #sq6
    X2[j,11]=LO[21*j+12] #sq7
    X2[j,12]=LO[21*j+13] #sq8
    X2[j,13]=LO[21*j+14] #gluino
    X2[j,14]=LO[21*j+17] #predLO
    y1[j]=LO[21*j+15] #k
...
for i in range(0, cnt):
        ...
        X1[i,3]=X2[i,3]
        X1[i,4]=abs(X2[i,4])
        X1[i,5]=(X2[i,5]+X2[i,6]+X2[i,7]+X2[i,8]+X2[i,9]+X2[i,10]+X2[i,11]+X2[i,12])/8
        X1[i,6]=X2[i,13]
...
history = History()
checkpointer = ModelCheckpoint(filepath="c1c1_K.hdf5", verbose=1, save_best_only=True)
early_stopping = EarlyStopping(monitor='val_loss', patience=50)

tlog=open("c1c1_K.txt", "w")
learnrate=0.001
pat=50
iterations = 7
epochs = 250
...
while learnrate > lr_limit:
	...
	checkpointer = ModelCheckpoint(filepath='c1c1_K_' + str(k) + '.hdf5', verbose=1, save_best_only=True)
	...
	model.load_weights('c1c1_K_' + str(k) + '.hdf5')
	learnrate /= c_lr
	k=k+1
