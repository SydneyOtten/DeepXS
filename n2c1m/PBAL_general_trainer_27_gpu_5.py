...
y_AL = np.reshape(y_AL, (3450000, 1))
...
mapes = open('27_general_mapes_gpu_5.txt', 'w')
while learnrate > lim:
	checkpointer = ModelCheckpoint(filepath="27_general_gpu_5_" + str(j) + ".hdf5", verbose=1, save_best_only=True)
	...
	plt.tight_layout()
	plt.savefig('27_general_gpu_5_' + str(j) + '.png')
	plt.close()
