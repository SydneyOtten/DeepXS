...
# for i in range(0, N):
        ...
        # else:
            ...
            # X1[i,9]=abs(X2[i,8])
        # X1[i,10]=X2[i,9]
        # X1[i,11]=(X2[i,11]+X2[i,12]+X2[i,13]+X2[i,14]+X2[i,15]+X2[i,16]+X2[i,17]+X2[i,10])/8
        # X1[i,12]=X2[i,18]
...
tlog=open("KNet27_8_correctLOs.txt", "w")
learnrate=0.001
pat=50
iterations = 7
epochs = 150
...
while learnrate > lr_limit:
	...
	checkpointer = ModelCheckpoint(filepath='weightsK27_8_' + str(k) + '.hdf5', verbose=1, save_best_only=True)
	...
	model.load_weights('weightsK27_8_' + str(k) + '.hdf5')
	learnrate /= c_lr
	k=k+1
	...
	plt.tight_layout()
	plt.savefig('27_K_general_8_' + str(k) + '.png')
	plt.close()
